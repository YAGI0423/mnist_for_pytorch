...
class AlphaO:
    ...
    def create_model(self):
        def residual_module(x):
            y = K.layers.Conv2D(filters=256, kernel_size=3, padding='same', use_bias=False)(x)
        ...
        out = K.layers.Conv2D(filters=256, kernel_size=3, padding='same', use_bias=False)(input_layer) #, kernel_regularizer=l2(self.weight_decay)
        ...
        pnn = K.layers.Conv2D(filters=2, kernel_size=1, padding='same', use_bias=False)(out1)
