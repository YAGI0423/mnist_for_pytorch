...
class Rule:
    def __init__(self, board_size, win_seq_num=5):
        self.board_size = board_size
        self.win_seq_num = win_seq_num
    ...
    def game_status(self, list_board):
        ...
        #이전까지는 승패 결정 불가
        if len(list_board) < self.win_seq_num * 2 - 1: return 0;   #during

        #변수 선언================================
        last_x, last_y = list_board[-1]
        cut_size = self.win_seq_num - 1
        ...
        cropped_board = crop_board(
            self.rule.get_square_board(list_board),
            (last_x, last_y),
            cut_size
        )
